...
**  请求体  **: {json.dumps(request_data.get('json', request_data.get('data', {})), indent=2, ensure_ascii=False)}"""
    ...
    def _execute_api_request(self):
        """执行API请求 - 内联变量替换与提取，不依赖 ExtractUtil 避免 jsonpath 包冲突"""
        import os
        import re
        import requests
        ...
        # ---- 工具函数 ----
        def _load_ctx():
            """从 redis 或 extract.yaml 加载当前上下文变量字典"""
        ...
        def _save_ctx(key, value):
            """将提取到的变量写回 redis 或 extract.yaml"""
            backend = os.getenv("APIFRAME_CONTEXT_BACKEND", "file").lower()
            if backend == "redis":
                import redis as _redis
                redis_url = (os.getenv("APIFRAME_REDIS_URL")
                ...
                from apiframetest.commons import yaml_util
                yaml_util.write(setting.extract_path, {key: value}, mode='a+')

        def _yaml_read(key):
            """从 redis 或 extract.yaml 读取单个变量值"""
            key = key.strip()
            backend = os.getenv("APIFRAME_CONTEXT_BACKEND", "file").lower()
            if backend == "redis":
                import redis as _redis
                ...
                try:
                    val = _yaml.safe_load(open(setting.extract_path, encoding="utf-8")) or {}
                    return val.get(key)
                except Exception:
                    return None

        def _hot_replace(data_str):
            """替换 ${func(args)} 格式，仅内联支持 yaml_read，其余函数降级跳过"""
            regexp = "\\$\\{(.*?)\\((.*?)\\)\\}"
            func_list = re.findall(regexp, data_str)
            for func in func_list:
                func_name = func[0].strip()
                func_args = func[1].strip()
                new_val = None
                if func_name == 'yaml_read':
                    new_val = _yaml_read(func_args)
                # 其他函数可在此扩展，暂不支持则保留原占位符
                if new_val is None:
                    continue
                if isinstance(new_val, str) and new_val.isdigit():
                    new_val = int(new_val)
                old_token = '${' + func[0] + '(' + func[1] + ')}'
                data_str = data_str.replace(old_token, str(new_val))
            return data_str

        def _resolve_request(req_dict):
            """先热替换 ${func()}, 再用上下文替换 ${varName}"""
            req_str = _yaml.safe_dump(req_dict)
            req_str = _hot_replace(req_str)
            ctx = _load_ctx()
            if ctx:
                req_str = Template(req_str).safe_substitute(ctx)
            return _yaml.safe_load(req_str)

        def _extract_value(response, attr_name, expr, index):
            """从响应中按 jsonpath 或正则提取值，使用 jsonpath-ng"""
        ...
        # ---- 1. 替换请求中的占位符 ----
        request_config = _resolve_request(self.case_data["request"])
        ...
        # ---- 2. 发送请求 ----
        # 禁用系统代理：通过 trust_env=False 阻止 requests 读取系统环境变量中的代理配置
        # 避免 Celery worker / pytest 子进程中 IDE 代理端口不可用导致 ProxyError
            ...
            for var_name, rule in extract_rules.items():
                ...
                try:
                    ...
                    if value is not None:
                        _save_ctx(var_name, value)
                        allure.attach(
                            f"{var_name} = {value}",
                            name=f"变量提取: {var_name}",
                            attachment_type=allure.attachment_type.TEXT
                        )
                except Exception as e:
                    allure.attach(
